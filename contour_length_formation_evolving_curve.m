...
sim.c = zeros(2*sim.N, 1);
for i = 1:2*sim.N
    
    if mod(i, 2)==0
        sim.c(i) = normrnd(1.5*param.coff(4*param.Ns+2),0.1);
    else
        sim.c(i) = normrnd(1.5*param.coff(4*param.Ns+1),0.1);
    end
...
for i=0:sim.dt:sim.t
    ...
    [ctrl.u, sim.coff, sim.dcoff] = controller(ctrl.k, sim.L, sim.Gs,...
                                         4*param.Ns+2, sim.c, j*param.coff);
    norm(sim.dcoff);
    ...
    % plot real-time data
    data.cds = j*param.Gs*param.coff;
    data.cs = param.Gs*sim.coff;
    
    plot(data.cds(1:2:end), data.cds(2:2:end),  'k', ...
        data.cs(1:2:end), data.cs(2:2:end), 'r',...
        data.c(1:2:end, end), data.c(2:2:end, end), 'b*', 'LineWidth', 1);
%     plot(data.cs(1:2:end), data.cs(2:2:end), 'LineWidth', 1, 'Color', 'k'); hold on;
%     plot(data.c(1:2:end, end), data.c(2:2:end, end), 'b*',  'LineWidth', 1); 
    axis([-11, 9, -8, 12]);
    grid on;
    xlabel('x positions');  ylabel('y positions');
    legend('Desired Contour', 'Actual Contour', 'Agents Positions', 'Location', 'NorthWest');
    drawnow;
...
%% plotting
figure(1)
plot(1.5*data.cd(1:2:end), 1.5*data.cd(2:2:end), 'LineWidth', 1, 'Color', 'k');
hold on; grid on;
plot(data.cds(1:2:end), data.cds(2:2:end), 'LineWidth', 1, 'Color', 'r');
plot(data.c(1:2:end, 1), data.c(2:2:end, 1), 'b*',  'LineWidth', 1); hold on;
plot(1.5*data.ref(1:2:end), 1.5*data.ref(2:2:end), 'ko', 'LineWidth', 1); hold on;
